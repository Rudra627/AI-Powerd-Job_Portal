...
export default function Profile() {
  ...
  const changePhoto = (e) => {
    ...
    if (file.size > 2.8 * 1024 * 1024) {
      toast.error("Upload image size should not exceed 3MB");
      return;
    }
    ...
    reader.onloadend = () => {
      const img = new Image();
      img.onload = () => {
        // Compress image to ensure the base64 payload is small and never trips backend limit during profile edits
        ...
        const photoStr = canvas.toDataURL("image/webp", 0.8);

        if (photoStr.length > 3 * 1024 * 1024) {
          toast.error("Upload image size is too large.");
          return;
        }
